...
struct BLOCK block[19] = {   //初始化各个游戏方块
		{ 1, 1, 1, 2, 1, 3, 2, 3, RED, 1 }, { 0, 2, 1, 2, 2, 2, 0, 3, RED, 2 },
				{ 0, 1, 1, 1, 1, 2, 1, 3, RED, 3 }, { 2, 1, 0, 2, 1, 2, 2, 2,
						RED, 0 }, { 1, 1, 1, 2, 0, 3, 1, 3, ORANGE, 5 }, { 0, 1,
						0, 2, 1, 2, 2, 2, ORANGE, 6 }, { 1, 1, 2, 1, 1, 2, 1, 3,
						ORANGE, 7 }, { 0, 2, 1, 2, 2, 2, 2, 3, ORANGE, 4 }, { 1,
						1, 0, 2, 1, 2, 2, 2, YELLOW, 9 }, { 1, 1, 1, 2, 2, 2, 1,
						3, YELLOW, 10 }, { 0, 2, 1, 2, 2, 2, 1, 3, YELLOW, 11 },
				{ 1, 1, 0, 2, 1, 2, 1, 3, YELLOW, 8 }, { 1, 1, 1, 2, 2, 2, 2, 3,
						GREEN, 13 }, { 1, 2, 2, 2, 0, 3, 1, 3, GREEN, 12 }, { 2,
						1, 1, 2, 2, 2, 1, 3, CYAN, 15 }, { 0, 2, 1, 2, 1, 3, 2,
						3, CYAN, 14 },
				{ 1, 0, 1, 1, 1, 2, 1, 3, LIGHT_BLUE, 17 }, { 0, 2, 1, 2, 2, 2,
						3, 2, LIGHT_BLUE, 16 }, { 1, 1, 2, 1, 1, 2, 2, 2,
						PURPLE, 18 } };
void Paint(HDC hdc, HPEN hpen) //此函数用于初始化界面
{
	...
	HBRUSH hbrush; //定义画刷
	hbrush = CreateSolidBrush(BgColor ); //给画刷赋初值，画刷颜色采用背景色
	...
	for (i = 1; i < H - 1; i++)  //绘制游戏区域方格线
		for (j = 1; j < W - 1; j++)
			Rectangle(hdc, j * BSIZE, i * BSIZE, (j + 1) * BSIZE,
					(i + 1) * BSIZE);
	for (i = 1; i < 5; i++)  //绘制右边状态栏游戏预览区域方格线
		for (j = W + 1; j < W + W1 - 1; j++)
			Rectangle(hdc, j * BSIZE, i * BSIZE, (j + 1) * BSIZE,
					(i + 1) * BSIZE);
	Rectangle(hdc, (W + 1) * BSIZE, Y2 * BSIZE, (W + W1 - 1) * BSIZE,
			(Y2 + 2) * BSIZE); //绘制分数栏方格线
	Rectangle(hdc, (W + 1) * BSIZE, Y3 * BSIZE, (W + W1 - 1) * BSIZE,
			(Y3 + 2) * BSIZE); //绘制等级栏方格线
	Rectangle(hdc, (W + 1) * BSIZE, Y4 * BSIZE, (W + W1 - 1) * BSIZE,
			(Y4 + 4) * BSIZE); //绘制帮助栏方格线
...
{
	char help1[] = "↑ - 旋转", help2[] = "↓ - 下移", help3[] = "← - 左移", help4[] =
			"→ - 右移";
...
void EraseBox(HDC hdc, HPEN hpen, int x, int y, int num) //清除(x,y)处编号为num，的方块
{
	...
	for (i = 0; i < 4; i++)   //用背景色填充方块所在区域，使方块隐藏
		Rectangle(hdc, (x + block[num].a[i][0]) * BSIZE,
...
void SetFullRow(HDC hdc, HPEN hpen) //满行处理函数
{
	...
	if (flag) //如果有满行，则重绘主板
	{
		for (i = org_top; i < y + 4; i++) //原来的最顶端
			...
			if (i <= 0 || i >= H - 1)
				continue; //越界了，就跳出本次循环
			for (j = 1; j < W - 1; j++) { //注意这里绘制主板时，每次都要选择不同的画刷，用完后一定要删除
...
void ChangeVar(void) //改变游戏主板的状态
{
	int i;
	for (i = 0; i < 4; i++) {
		board[y + block[cur_boxnum].a[i][1]][x + block[cur_boxnum].a[i][0]].var =
				1; //状态置1,表示有方块填充
		board[y + block[cur_boxnum].a[i][1]][x + block[cur_boxnum].a[i][0]].color =
				block[cur_boxnum].color;
	}
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam) { //定义窗口过程函数
	int i, j;
